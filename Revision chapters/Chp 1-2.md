...
o	Endpoint protection and OS logs: phishing email, lateral movement
o	Server logs and network captures: unauthorized or malicious process
o	Firewall logs and network capture: data extraction and submission
...
o	One of the most valuable pieces of info that you have in the post-incident activity phase
o	Helps to keep refining process through identification of gaps in the current process and areas of improvement
o	Documentation must be very detailed with full timeline of incident
o	Content: steps were taken to resolve problem, what happened during each step and how issue was resolved outlined in depth
•	Lesson learnt will include answers of the following
o	Who identified the sec issue === user or detection system
o	Was incident opened with right priority
o	Did sec ops team perform initial assessment correctly
o	Was data analysis done correctly
o	Were containment, eradication and recovery done correctly
o	Anything that could be improved at this point
o	How long did it take to resolve this incident
•	Evidence retention
o	All artifacts should be stored according to company’s retention policy
o	Evidence must be kept intact until legal actions are completely settled

IR cloud update
•	Preparation
o	Needs to update contact list to include cloud provider contact info, on-call process, etc
•	Detection
o	Include cloud provider solution for detection in order to assist during investigation
•	Containment
o	Revisit cloud provider capabilities to isolate an incident (eg. Isolate compromised VM for others)
...
o	7 domains include: User domain, workstation domain, LAN domain, LAN-to-WAN domain, remote access domain, WAN domain and System/Application domain
•	Collection of security event and alarm data
•	Collection of log and machine data
•	Collection of forensic sensor data
Discovery phase
•	Search analytics
o	Carrying out software-aided analytics
o	Review reports and identify any known or reported exceptions from network and antivirus security tools
o	Labour-intensive -> shouldn’t be sole analytics method
•	Machine analytics
o	Purely done by machines/software
o	Autonomously scan large amounts of data and give brief simplified results to people using ML
Qualification phase
•	Threats are assessed to find out
o	Potential impact
o	Urgency of resolution
o	How to mitigate the threats
...
o	Changes caused by attacker or for recover are needed to be backtracked
...
o	Attacker harvest as much info as possible to find vulns
o	Decides on exploitation techniques that are suitable for each vuln
...
o	Social engineering attacks such as phishing is used to get entry point of the org network
o	Phishing is usually linked to malware installation and claims to be from reputable institutions
	Other types of social engineering attacks: closely follow targets and collect info (happens through social media)
•	Compromising the system
o	Once either of these or another technique is used, attacker will find point of entrance through stolen passwords or malware infection
o	Stolen pw will give attacker direct access to pc, servers or devices within internal network of org
o	Malware can be used to infect even more pc or server which brings them under the command of the hacker
•	Lateral movement
o	This phase involves use of various scanning tools to find loopholes that can be exploited to stage an attack
o	Pop scanning tools (framework): Metasploit and Kali Linux
o	(network): Wireshark, Nmap, Aircrack-ng, Kismet, OWASP Zap
...
o	Pw cracking tools: John the Ripper, THC Hydra and Cain & Abel -> supports brute or dictionary attacks on pw
•	Privilege escalation
o	Vertical priv esc is where attacker moves from one acc to another that has higher level of auth and tools are used to esc priv
...
o	Horizontal priv esc is where the attacker uses the acc that has the same level of auth and that acc is used to esc priv
